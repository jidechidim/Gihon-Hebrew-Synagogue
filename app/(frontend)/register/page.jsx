...
export default function RegisterEventPage() {
  ...
  useEffect(() => {
    async function fetchEvents() {
      try {
        ...
        // Preselect if eventId exists
        if (preselectId && data?.length) {
          const match = data.find((ev) => ev.id === preselectId);
          if (match) setFormState((prev) => ({ ...prev, eventSelection: match.title }));
        }
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (formState.specialRequirements) formData.append("Special Requirements", formState.specialRequirements);
  ...
  if (loading) return <p className="container narrow center mt-10">Loading events...</p>;
  if (error) return <p className="container narrow center mt-10 text-red-600">{error}</p>;

  return (
    <main className="page">
      <section className="container narrow center form-section">
        <h1>Event Registration</h1>
        <p className="subtitle">Please fill out this form to register for an event.</p>
        ...
        <form className="gihon-form" onSubmit={handleSubmit}>
          ...
          <label>
            <input
              ...
              onChange={handleChange}
            /> I consent to being contacted with updates about this event <span className="required">*</span>
          </label>
